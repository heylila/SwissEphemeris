...
final class UranusRetrograde: XCTestCase {
    ...
    func testUranusRetrograde() throws {
        ...
        for i in stride(from: sliceIndex, to: slices.endIndex, by: 1) {
            ...
            let window = findRetrogradeDates(target: Planet.uranus.celestialObject, past, future, time)
        ...
        // 2022-08-24 to 2023-01-22
        XCTAssert(past.component(.month)! == 8, "Erroneoes Rx starting month is \(past.component(.month)!)")
        XCTAssert(past.component(.day)! == 24, "Erroneoes Rx starting day is \(past.component(.day)!)")
