...
class MoonPhases: XCTestCase {
    ...
    func testFindNextNewMoon() throws {
        ...
        var sunPositions = BodiesRequest(body: Planet.sun.celestialObject).fetch(start: originDate, end: endDate, interval: interval)
        var moonPositions = BodiesRequest(body: Planet.moon.celestialObject).fetch(start: originDate, end: endDate, interval: interval)

        XCTAssert(sunPositions.count == moonPositions.count)

        var sun2Moon: [(sun: Coordinate, moon: Coordinate)] = Array(zip(sunPositions, moonPositions))
        ...
        sunPositions = BodiesRequest(body: Planet.sun.celestialObject).fetch(start: newMoonTestStart, end: newMoonTestEnd, interval: TimeSlice.minute.slice)
        moonPositions = BodiesRequest(body: Planet.moon.celestialObject).fetch(start: newMoonTestStart, end: newMoonTestEnd, interval: TimeSlice.minute.slice)
    ...
    func testFindNextFullMoon() throws {
        ...
        sunPositions = BodiesRequest(body: Planet.sun.celestialObject).fetch(start: fullMoonTestStart, end: fullMoonTestEnd, interval: TimeSlice.minute.slice)
        moonPositions = BodiesRequest(body: Planet.moon.celestialObject).fetch(start: fullMoonTestStart, end: fullMoonTestEnd, interval: TimeSlice.minute.slice)
...
//        }
    ...
    func testFailedAugustFirstQuarterMoon() throws {
        ...
        var suns = BodiesRequest(body: Planet.sun.celestialObject).fetch(start: startSearch, end: endSearch, interval: hourSlice)
        var moons = BodiesRequest(body: Planet.moon.celestialObject).fetch(start: startSearch, end: endSearch, interval: hourSlice)
        ...
        suns = BodiesRequest(body: Planet.sun.celestialObject).fetch(start: startSearchMin, end: endSearchMin)
        moons = BodiesRequest(body: Planet.moon.celestialObject).fetch(start: startSearchMin, end: endSearchMin)
    ...
    func testFindNextFirstQuarterMoon() throws {
        ...
        var suns = BodiesRequest(body: Planet.sun.celestialObject).fetch(start: newMoonDate, end: fullMoonDate, interval: slice)
        var moons = BodiesRequest(body: Planet.moon.celestialObject).fetch(start: newMoonDate, end: fullMoonDate, interval: slice)
        ...
        suns = BodiesRequest(body: Planet.sun.celestialObject).fetch(start: startMinSearch, end: endMinSearch)
        moons = BodiesRequest(body: Planet.moon.celestialObject).fetch(start: startMinSearch, end: endMinSearch)
    ...
    func testFindNextTwoNewMoons() throws {
        ...
        let hourSlice = TimeSlice.hour.slice
        var sunPositions = BodiesRequest(body: Planet.sun.celestialObject).fetch(start: originDate, end: endDate1, interval: hourSlice)
        var moonPositions = BodiesRequest(body: Planet.moon.celestialObject).fetch(start: originDate, end: endDate1, interval: hourSlice)
        ...
        sunPositions = BodiesRequest(body: Planet.sun.celestialObject).fetch(start: newMoonTestStart, end: newMoonTestEnd, interval: minSlice)
        moonPositions = BodiesRequest(body: Planet.moon.celestialObject).fetch(start: newMoonTestStart, end: newMoonTestEnd, interval: minSlice)
        ...
        sunPositions = BodiesRequest(body: Planet.sun.celestialObject).fetch(start: originDate2, end: endDate2, interval: hourSlice)
        moonPositions = BodiesRequest(body: Planet.moon.celestialObject).fetch(start: originDate2, end: endDate2, interval: hourSlice)
        ...
        sunPositions = BodiesRequest(body: Planet.sun.celestialObject).fetch(start: newMoonStart2, end: newMoonEnd2, interval: minSlice)
        moonPositions = BodiesRequest(body: Planet.moon.celestialObject).fetch(start: newMoonStart2, end: newMoonEnd2, interval: minSlice)
        sun2Moon = Array(zip(sunPositions, moonPositions))
        newMoonPos = sun2Moon.min { lhs, rhs in
            return lhs.sun.longitudeDelta(other: lhs.moon) < rhs.sun.longitudeDelta(other: rhs.moon)
        }
    ...
    func testFindNextWaxingCrescent() throws {
        ...
        suns = BodiesRequest(body: Planet.sun.celestialObject).fetch(start: newMoonDate, end: fullMoonDate, interval: slice)
        moons = BodiesRequest(body: Planet.moon.celestialObject).fetch(start: newMoonDate, end: fullMoonDate, interval: slice)
